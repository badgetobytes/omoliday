...
//
// Omoliday is the stock Omarchy clock (this file and Panel.qml start from
// its source) with public holidays marked on the calendar. The label, the
// format ring and the popup contract are unchanged; the holiday work lives
// in Panel.qml, Holidays.js and HolidayFetch.qml.
//
...
// middle click opens the timezone picker, the way the stock clock does: the
// one fixed command name is handed to the bar's own run facade, which the
// shell executes for every stock widget alike. Nothing here builds a
// command string.
BarWidget {
  id: root
  ...
  // Holiday state for a terminal: `omarchy-shell fstander.omoliday holidays`
  // reports the country, cache and last fetch outcome as JSON, and
  // refetchHolidays drops the cache and asks the source again.
  function holidayDiagnostics() {
    if (panelLoader.item && panelLoader.item.holidayDiagnostics) return panelLoader.item.holidayDiagnostics()
    return "{}"
  }
  ...
  IpcHandler {
    ...
    function holidays(): string { return root.holidayDiagnostics() }
    // Every instance drops its cache, like refresh; the one that fetches
    // saves the new years and the rest pick them up from the saved entry.
    function refetchHolidays(): void { root.broadcast("refetchHolidays") }
  }
